...
module CASServer
  class Server < CASServer::Base
    ...
    before do
      ...
      @custom_views = ENV['CUSTOM_VIEWS'] || settings.config[:custom_views]
      @template_engine = settings.config[:template_engine] || 'haml'
    ...
    get "#{uri_path}/login" do
      ...
      if tgt and !tgt_error
        @authenticated = true
        if not @service
          login_email tgt.username
          redirect "/"
        end

        @authenticated_username = tgt.username
        @message = {:type => 'notice',
          :message => "Logged in as #{tgt.username}"}
      elsif tgt_error
    ...
    post "#{uri_path}/login" do
      ...
      # 2.2.1 (optional)
      @service = clean_service_url(params['service']) || "/"
      puts "Service #{@service}, params: #{[params['service']]}"
      ...
      begin
        ...
        if credentials_are_valid
          ...
          if @service.blank?
            $LOG.info("Redirecting to / as no service given")
            return redirect "/"
...
#    end
    ...
    get "#{uri_path}/logout" do
      ...
      @message = {:type => 'confirmation', :message => "logged out"}

      @message[:message] += "click to continue" if @continue_url
